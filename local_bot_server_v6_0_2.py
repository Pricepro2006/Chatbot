...
# ---------------------------------------------------------------------------
# Blacklist of common question words to prevent false synonyms
STOPWORDS = {"which", "what", "who", "how", "and", "or", "the", "is", "are", "does", "do", "to", "in", "of", "for"}
...
def extract_skus(text: str) -> list[str]:
    # Extract only candidates that contain at least one digit (avoids matching pure words)
    pattern = "\b(?=[A-Z0-9]*\d)[A-Z0-9]{5,}(?:[/\-\s][A-Z0-9]{1,3})?\b"
    candidates = re.findall(pattern, text.upper())
    # Whitelist filter: only keep SKUs present in DEALS_DF
    valid_bases = {
        fnorm.normalize_sku(s).upper()
        for s in DEALS_DF["Product number"].dropna().astype(str)
    }
    skus = [sku for sku in candidates if fnorm.normalize_sku(sku).upper() in valid_bases]
    return skus

# ---------------------------------------------------------------------------

def find_fields(text: str) -> list[str]:
    # Use LLM to suggest candidate fields, then filter to our canonical set
    raw = get_fields(text)
    fields: list[str] = []
    for f in raw:
        ...
        if can in FIELD_ROUTER:
            fields.append(can)
    return fields  # no default fallback; let caller handle empty list
...
def ask():
    ...
    deal = extract_deal(q)
    # Step 1: use LLM for field detection and learn new synonyms
    raw_fields = get_fields(q)
    fields: list[str] = []
    for raw in raw_fields:
        # skip common stopwords to avoid false synonyms
        if raw.strip().lower() in STOPWORDS:
            continue
        canon = fnorm.normalize_field_name(raw)
        if canon in FIELD_ROUTER:
            # Learn this new mapping into the golden brain
            try:
                add_synonym(raw, canon)
            except Exception:
